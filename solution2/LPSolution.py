...
class LPSolution(BaseSolution):
    ...
    def solve(self, c, T, t, f):
        ...
        # optimize target (4a)
        prob += sum([c[i] * y[i] for i in xrange(self.n)] + [f[i][j] * x[i][j] for i in xrange(self.n) for j in xrange(self.m)])

        # 4b
        for j in xrange(self.m):
            prob += sum([x[i][j] for i in xrange(self.n)]) >= 1.0
        ...
        # 4d
        for i in xrange(self.n):
            prob += T[i] * y[i] >= sum([t[i][j] * x[i][j] for j in xrange(self.m)])
